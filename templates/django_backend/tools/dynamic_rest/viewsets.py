...
from tools.dynamic_rest.conf import settings
from tools.dynamic_rest.filters import DynamicFilterBackend, DynamicSortingFilter
from tools.dynamic_rest.metadata import DynamicMetadata
from tools.dynamic_rest.pagination import DynamicPageNumberPagination
from tools.dynamic_rest.processors import SideloadingProcessor
from tools.dynamic_rest.utils import is_truthy
...
class PaginationParams(serializers.Serializer):
...
class ViewSetReturnTypeItem:
    def __init__(self, item_key, item_type, many):
        self.item_key = item_key
        self.item_type = item_type
        self.many = many


class ViewSetReturnType:
    def __init__(self, items: List[ViewSetReturnTypeItem]):
        self.items = items


pagination_params = ViewSetReturnTypeItem(
    item_key="meta",
    item_type=PaginationParams(),
    many=False,
)
...
class WithDynamicViewSetMixin(object):
    ...
    def supports_bulk(self) -> bool:
        # TODO: Other actions have bulk support
        if self.action == "create":
            return True
        return False

    def auto_get_serializer_class(self, serializers):
        base_name = self.serializer_class.__name__
        if self.action == "create":
            expect = f"Create{base_name}"
        elif self.action in ["list", "retrieve", "destroy"]:
            expect = base_name
        elif self.action == "partial_update":
            expect = f"Update{base_name}"
        else:
            assert False, f"Unhandled action {self.action}"

        if hasattr(serializers, expect):
            return getattr(serializers, expect)

        assert False, f"{expect} is not implemented within {serializers}"

    def get_return_type(self) -> ViewSetReturnType:
        if self.action in ["update", "create", "partial_update"]:
            item_type = self.serializer_class
        else:
            item_type = self.get_serializer_class()

        many = self.action == "list"
        items = [
            ViewSetReturnTypeItem(
                item_key=(
                    item_type.get_plural_name() if many else item_type.get_name()
                ),
                item_type=item_type,
                many=many,
            )
        ...
        if self.action == "list":
            items.append(pagination_params)

        return ViewSetReturnType(items=items)
    ...
    def handle_multipart_list_data(self, request):
        ...
        array_name_getters = [
            (
                k,
                lambda j, k=k: f"{k}[{j}]",  # Check for FormData array syntax field_1[0], field_2[0], field_1[1], field_2[1]
            )
            for k in serializer_class().get_fields().keys()
        ]
        while any(getter(i) in request.data for _, getter in array_name_getters):
            item_data = {k: request.data.get(g(i)) for k, g in array_name_getters}
        ...
        if data_list:
            print(f"LIST DATA PARSED: {data_list}")
            many = True
            data = data_list
        else:
            print(f"USING NORMAL DATA: {request.data}")
            many = False
            data = request.data
